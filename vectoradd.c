...
int main() {
    ...
    status = clGetEventProfilingInfo(read_buf_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start_time, NULL);
    status = clGetEventProfilingInfo(read_buf_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end_time, NULL);
    printf("Execution time of read: %d nano seconds\n", end_time-start_time);

    status = clGetEventProfilingInfo(write_buf_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start_time, NULL);
    status = clGetEventProfilingInfo(write_buf_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end_time, NULL);
    printf("Execution time of write: %d nano seconds\n", end_time-start_time);
    
    status = clGetEventProfilingInfo(execution_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start_time, NULL);
    status = clGetEventProfilingInfo(execution_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end_time, NULL);
    printf("Execution time of kernels: %d nano seconds\n", end_time-start_time);    

    status = clReleaseEvent(read_buf_event);
    status = clReleaseEvent(write_buf_event);
    status = clReleaseEvent(execution_event);
